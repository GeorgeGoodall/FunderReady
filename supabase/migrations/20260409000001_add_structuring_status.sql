-- Add 'structuring' to application_reviews status CHECK constraint
-- This status is used by the pipeline when assigning sections to a long unstructured document
ALTER TABLE public.application_reviews
  DROP CONSTRAINT application_reviews_status_check;

ALTER TABLE public.application_reviews
  ADD CONSTRAINT application_reviews_status_check
    CHECK (status IN ('pending', 'structuring', 'analysing', 'cross_referencing', 'scoring', 'completed', 'failed'));

-- Update submit_review (without is_draft) to treat 'structuring' as in-progress
...
BEGIN
  ...
  IF EXISTS (
    ...
    AND ar.status IN ('pending', 'structuring', 'analysing', 'cross_referencing', 'scoring')
  ) THEN
...
-- Update submit_review (with is_draft) to treat 'structuring' as in-progress
CREATE OR REPLACE FUNCTION submit_review(
  p_application_id UUID,
  p_user_id UUID,
  p_review_number INT,
